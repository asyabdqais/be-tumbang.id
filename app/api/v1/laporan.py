...
from app.models.antropometri import Antropometri
from typing import Optional
from datetime import date

router = APIRouter()

allow_laporan = RoleChecker([Role.ADMIN, Role.DOKTER])


@router.get("/ringkasan", dependencies=[Depends(allow_laporan)])
...
@router.get("/per-wilayah", dependencies=[Depends(allow_laporan)])
def get_laporan_per_wilayah(
    ...
    Data ini digunakan Dinas Kesehatan untuk alokasi program intervensi.
    """
    query = db.query(
        Balita.rw_desa,
        Antropometri.status_gizi,
    ...
    return {
        "filter": {"tahun": tahun},
        "data": list(wilayah_map.values())
    }


@router.get("/balita-berisiko", dependencies=[Depends(allow_laporan)])
